            ...
            if (result) {
                ...
                const sampleNextWindow = async () => {
                    ...
                    try {
                        ...
                        let r = ['Inferencing result', ''].concat(res.results.map(k => {
                            return k.label + ': ' + k.value.toFixed(2)
                        }));
                        if (res.anomaly !== 0.0) {
                            r.push('');
                            r.push('anomaly: ' + res.anomaly.toFixed(2));
                        }

                        this._elements.inferencingResult.textContent = r.join('\n');
